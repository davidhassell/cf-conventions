...
|  The name of the geodetic (horizontal) datum, which corresponds to the 
     ...
     *FIX THIS URL.  The referenced url should be both moved and updated. Once moved, update this
     and the other references to it.*  and are 
     ...
     (see http://home.gdal.org/projects/opengis/wktproblems.html for more details). 
...
// The following line shows how to insert Unicode for <=.  A plain '<=' turns into an arrow.
// An alternative is to use HTML <=, as &le;
...
| Specifies the longitude, with respect to Greenwich, of the prime
    ...
    Domain: **`-180.0 &le; longitude_of_prime_meridian &lt; 180.0`**
    decimal degrees.
    Default = **`0.0`**
...
| The longitude (degrees_east) chosen as the origin of rectangular 
    coordinates for a map projection.  
    Domain: **`-180.0 &le; longitude_of_projection_origin < 180.0`**
...
|   This indicates a list of up 
     ...
     array, with 3, 6 or 7 values (if there are fewer than 7 values the remaining are 
