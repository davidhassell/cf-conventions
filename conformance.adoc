...
* OK When attached to a data variable, the type of the
**`coordinate_interpolation`** attribute is a string whose value is a
list of blank separated word groups of the following form, in which
brackets indicate optional text: **`tie_point_variable:
[tie_point_variable: ...] interpolation_variable`**.  Each
**`tie_point_variable`** token specifies a tie point coordinate variable that
must exist in the file, and each **`interpolation_variable`** token
specifies an interpolation variable that must exist in the file.

* OK An interpolation variable must have one of the string-valued
...
Interpolation Methods section of
http://cfconventions.org/cf-conventions/cf-conventions.html#appendix-coordinate-subsampling[Appendix
J].

* OK An interpolation variable must have the attribute
**`computational_precision`**.  The legal values for the
**`computational_precision`** attribute are contained in the
Interpolation Method Implementation subsection of the Lossy
Compression by Coordinate Subsampling section of chapter 8.

* An interpolation variable must have a **`tie_point_mapping`**
  ...
  optional text: **`interpolated_dimension: tie_point_index_variable
  subsampled_dimension [interpolation_subarea_dimension]`**.  Each
  **`interpolation_dimension`** token specifies a unique interpolation
  dimension of the parent data variable, each
  **`tie_point_index_variable`** token specifies a unique tie point
  index variable, each **`subsampled_dimension`** token specifies the
  subsampled dimension spanned by the tie point index variable, and
  each **`interpolation_subarea_dimension`** token specifies a unique
  interpolation subarea dimension dimension. The subsampled dimensions
  and interpolation dimensions must not be dimensions of the parent
  data variable.

* OK The tie point variables associated with each interpolation
  variable token must all span the same dimensions, which comprise a
  subset of zero or more dimensions of the parent data variable with
  the addition of all of the tie point interpolation dimensions
  identified by the **`tie_point_mapping`** attribute of the
  interpolation variable.  A tie point variable must not span both a
  subsampled_dimension dimension and its corresponding interpolation
  dimension, as defined by the **`tie_point_mapping** attribute.
...
The specified variable must exist in the file.
* OK A bounds tie point variable must have the same dimensions as its associated tie points coordinate variable.
* OK A bounds tie point variable must be a numeric data type.
* OK A bounds tie point variable must not have the **`_FillValue`** or **`missing_value`** attributes.
...
* OK An interpolation variable should have 0 dimensions.
* OK The recommendations on bounds tie point variable attributes are the same as for bounds variables described in <<cell-boundaries>>.
