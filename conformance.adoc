...
* An uncertainty variable probability_distribution attribute must be a string attribute of the form: "distribution_name [(moment: value [moment: value ...])]". 

** The "distribution_name" keyword must be one of the probability distribution names defined in Appendix N. 
...
* If an uncertainty variable has an error_correlation attribute then it must be a string whose value is one or more blank separated word lists, each with one of the forms, where square brackets indicate optional content: 
  ...
  dim: error_correlation_var [(comment)] 
  
  dim: dim: [dim: ...] error_correlation_var [(comment)] 
  
  [dim:] error_correlation_structure [(comment)] 
  
  [dim:] error_correlation_structure (parameter: value [parameter: value ...] [comment]) 
