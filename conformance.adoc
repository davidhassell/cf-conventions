...
* If an uncertainty variable has an error_correlation attribute then it must be a string whose value is one or more blank separated word lists, each with one of the forms, where square brackets indicate optional content:
  ...
  [dim:] error_correlation_structure [(comment: comment)]

  [dim:] error_correlation_structure (parameter: value [parameter: value ...] [comment: comment])
...
* A value must either name an error-correlation parameter variable or else be a string representation of an integer.

* When a comment appears with a parameter: value pair, the comment must be preceded by the string "comment:"
