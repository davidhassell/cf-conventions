...
**`tie_points`** attribute may be defined for a data variable. This is
a string attribute that both identifies the tie point variables, and
maps non-overlapping subsets of them to their corresponding
...
by the interolation variable **`bi_linear`** could be indicated with
**`tp_lat: tp_lon: bi_linear`**.
