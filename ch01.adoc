...
aggregated data:: The data of an aggregation variable, after it has been created by an application program.

aggregated dimension:: A dimension of the aggregated data of an aggregation variable.

aggregation variable:: A variable whose data is defined by as a virtual aggregation of fragments, rather than containing its own data.
...
Note that an aggregation coordinate variable is stored as a scalar, but must have the same name its aggregated dimension (see <<aggregation-variables>>).

fragment:: A constituent part, found in an external file, of the aggregated data of an aggregation variable.
