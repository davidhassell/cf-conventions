...
Any variable may be an aggregation variable, and being an aggregation variable does not affect its CF role (data variable, coordinate variable, boundary variable, cell measure variable, etc.). The interpretation of variables and dimensions needs to account for the fact that the aggregated dimensions of an aggregation variable have exactly the same status as the dimensions of a normal (i.e. non-aggregation) variable. For instance:
