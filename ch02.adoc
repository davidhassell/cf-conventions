...
Nearly every kind of variable (data, auxiliary coordinate, boundary, cell measure, etc.) may be an aggregation variable, and being an aggregation variable does not change its CF function. The one exception is for coordinate variables, which are disallowed in order to maintain consistency with the
link:$$https://docs.unidata.ucar.edu/nug/current/best_practices.html#bp_Coordinate-Systems$$[NUG definition of coordinate variables].
...
`[180, 360)` degrees east
|===============
Six fragments are arranged in a three-dimensional fragment array with shape `(1, 3, 2)`. Each fragment spans the entirety of the Z dimension, but only a part of the Y-X plane. They may be combined to create a three-dimensional aggregated data array that has global `(Z, Y, X)` coverage, with shape `(17, 181, 360)`.
...
The fragment array is defined by the aggregation variable's **`aggregated_data`** attribute. This attribute takes a string value comprising blank-separated elements of the form "__feature: variable__", where __feature__ is a case-sensitive keyword which identifies a feature of the fragment array, and __variable__ is a __fragment array variable__ that provides the feature's values for each fragment. The order of elements is not significant.

There are four standardized and mandatory fragment array features, identified by the `file`, `format`, `address`, and `shape` features; and any amount of non-standardized features are also allowed:
...
The `address` fragment array variable defines how to find the fragments within the fragment files. In general it has the same shape as the `file` fragment array variable, and must contain a non-missing value for each fragment. However, if the `address` fragment array variable is a scalar, then its single value is assumed to apply to all fragments. It may have any data type. For a CF-netCDF fragment file, an address must be the fragment's netCDF variable name. Addresses for other fragment file formats are allowed, on the understanding that an application program may choose to ignore any values that it does not understand.
    
`shape`

The integer-valued `shape` fragment array variable defines the shape of each fragment in its canonical form (see <<fragment-interpretation>>). In general, the `shape` fragment array variable is two-dimensional, with the size of the slower varying dimension (i.e. the number of rows) being the number of fragment dimensions, and the size of more rapidly varying dimension (i.e. the number of columns) being the size of the largest fragment dimension. Each row provides the sizes of the fragments along that dimension of the fragment array. For each fragment dimension that is smaller than the largest fragment dimension, the columns are padded with missing values. When the aggregated data is a scalar there are no aggregated dimensions, and the `shape` fragment array variable variable must be one-dimensional, of size one, and contain the value `1`.

*Non-standardized features*

Any amount of non-standardized features are allowed, on the understanding that an application program may choose to ignore any such features that it does not understand, or which are irrelevant for its purpose. In general the fragment array variable for a non-standardized feature has the same shape as the `file` fragment array variable, and its values are assumed to apply to the corresponding fragments. However, if the fragment array variable is a scalar, then its single value is assumed to apply to all fragments.

Use cases for non-standardized features include, but are not limited to:

* To provide extra information that enable the aggregation of fragments stored in a file format for which the `address` fragment array variable alone is insufficient to identify the fragments within the fragment files.

* To store extra metadata that relate to the fragments, but which are not necessary for the creation of the aggregated data. For instance, it may be convenient to store an attribute from each fragment file so that it is available without having to open and inspect the fragment files themselves. See <<example-L.7>>.
...
* The fragment's data has the same units as the aggregated variable. 

* The fragment's data has the same indication of missing values as the aggregated variable.

The conversion of fragments to their canonical form is the responsibility of the application program which is creating the aggregated data, and should include the following operations:
...
An application program may choose to implement only a subset of these operations, but it must not create the aggregated data if can't convert all of the fragments to their canonical form.