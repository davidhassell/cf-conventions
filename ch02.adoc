...
An __aggregation variable__ is a variable which has been formed by combining (i.e. aggregating) multiple __fragments__ stored in pre-existing __fragment files__ that are external to the file containing the aggregation variable. i.e. the __aggregation file__. The aggregation variable does not contain any actual data, instead it contains instructions on how to create its __aggregated data__ as an aggregation of the data from each fragment.

Aggregation provides the utility of being able to view, as a single entity, a dataset that has been partitioned across multiple files, whilst taking up very little space on disk (since the aggregation variable has no copy of the data in the fragments). The fragment files may be CF-compliant or have any other format, thereby allowing an aggregation variable to act as CF-compliant view of non-CF datasets. Storing aggregations is useful for data analysis, as it avoids the computational expense of deriving the aggregation at the time of analysis; and for archive curation, as the aggregation can act as a metadata-rich archive index.

An aggregation variable should be a scalar (i.e. it has no dimensions) and the value of its single element is immaterial. It acts as a container for all of the usual attributes that describe the data, with the addition of two special attributes: one to define the _aggregated dimensions_, i.e. the dimensions of the aggregated data; and one to provide instructions on how the aggregated data is to be created. The data type of the aggregated data is the same as the data type of the aggregation variable.

Nearly every kind of variable (data, auxiliary coordinate, boundary, cell measure, etc.) may be an aggregation variable, and being an aggregation variable does not change its CF function. The one exception is for coordinate variables, which cannot be aggregation variables because they are must  

The details of how to encode and decode aggregation variables are given in this section, with annotated examples provided in <<appendix-aggregation-examples>>.

The conventions do not currently offer guidance on how to decide if two or more fragments can be aggregated in this way.
...
When two variables are expected to share common dimensions, e.g. as would be the case between a data variable and any of its coordinate or auxiliary coordinate variables, if either variable is an aggregation variable then its aggregated dimensions must be used instead of the usual variable dimensions.


[[fragment-array, Section 2.8.2, Fragment Array]]
==== Fragment Array

The fragments are conceptually organised into a __fragment array__ with the same number of dimensions as the aggregated data. Each dimension of the fragment array is called a __fragment dimension__, and corresponds to the aggregated dimension with the same position in the aggregated data. The size of a fragment dimension is equal to the number of fragments that span its corresponding aggregated dimension, of which there will always be at least one. The aggregated data is created by concatenating the fragments' data along each fragment dimension, in the order in which they appear in the fragment array.
...
Fragment data shape `(17, 45, 180)`
  ...
  * `[180, 360)` degrees east
|===============
Six fragments are arranged in a three-dimensional fragment array with shape `(1, 3, 2)`. Each fragment spans the entirety of the Z dimension, but only a part of the Y-X plane. They combine to create three-dimensional aggregated data that has global `(Z, Y, X)` coverage, with aggregated dimension sizes `(17, 181, 360)`.
====


[[aggregated-data, Section 2.8.3, Aggregation Data]]
==== Aggregated Data

The definitions of the fragments and the instructions on how to combine them into the aggregated data are provided by the aggregation variable's **`aggregated_data`** attribute. This attribute takes a string value comprising blank-separated elements of the form "__term: variable__", where __term__ is a case-sensitive keyword which identifies an aggregation instruction, and __variable__ is the name of an __aggregation instruction variable__ that provides information for the instruction. The order of elements is not significant.

There are four standardized and mandatory aggregation instructions, identified by the `file`, `format`, `address`, and `shape` terms; and any amount of non-standardized terms are also allowed:
...
Multiple versions of a fragment may be provided if an extra trailing dimension is included in the `file` aggregation instruction variable. Each version must contain equivalent information, so that any one of the versions may be selected for use in the aggregated data. This is useful when it known in advance that various locations will be possible for the fragment, but it is not known which of them will exist at any given future time. For instance, this feature could be used to define remotely stored and locally cached versions of a fragment, allowing an application program to commit to the expense of accessing the remote version only if the local version does not exist. If a fragment has fewer versions than others, then its trailing dimension must be padded with missing values.

To assist with fragment file management, a fragment file name may contain any number of string substitutions, each of which is defined by the `file` aggregation instruction variable's **`substitutions`** attribute. The use of substitutions can save space in the aggregation file; and in the event that the fragment files are moved from their original locations it may be possible for the fragment file names to be modfied by just editting the **`substitutions`** attribute, rather than by changing the `file` aggregation instruction variable values themselves. The **`substitutions`** attribute takes a string value comprising blank-separated elements of the form "__base: replacement__", where __base__ is a case-sensitive keyword that defines the part of a fragment file name which is to be replaced by the string defined by __replacement__ prior to locating and reading the fragment file. The order of elements is not significant. The _base_ keyword must have the form `${\...}`, where `\...` represents any characters. For instance, a fragment file name of `\file://data/store/file.nc` could also be stored as `${local}file.nc` in conjunction with `substitutions="${local}: \file://data/store/"`.
...
The `address` aggregation instruction variable defines how to find the fragments within the fragment files. In general it has the same shape as the `file` aggregation instruction variable, and must contain a value for each fragment. However, if the `address` aggregation instruction variable is a scalar, then its single value is assumed to apply to all fragments. For a CF-netCDF fragment file, the address must be the fragment's netCDF variable name. Address for other fragment file formats are allowed, on the understanding that an application program may choose to ignore any values that it does not understand.
    
`shape`

The integer-valued `shape` aggregation instruction variable defines the shape of each fragment in its canonical form (see <<fragment-interpretation>>). In general, the `shape` aggregation instruction variable is two-dimensional, with the size of the slower varying dimension (i.e. the number of rows) being the number of fragment dimensions, and the size of more rapidly varying dimension (i.e. the number of columns) being the size of the largest fragment dimension. Each row provides the sizes of the fragments along that dimension of the fragment array. For each fragment dimension that is smaller than the largest fragment dimension, the columns are padded with missing values. When the aggregated data is a scalar there are no aggregated dimensions, and the shape aggregation instruction variable variable must be one-dimensional, of size one, and contain the value 1.

*Non-standardized terms*

Any amount of non-standardized terms are also allowed on the understanding that an application program may choose to ignore any terms that it does not understand, or which are irrelevant for its purpose. Use cases for non-standardized terms include, but are not limited to:
...
[[fragment-interpretation, Section 2.8.4, Fragment Interpretation]]
==== Fragment Interpretation

The only restriction on the how a fragment is stored in a fragment file, of any format, is that the fragment must be convertible to its __canonical form__, and a fragment must be converted to its canonical form prior to being inserted into the aggregated data. It is up to the creator of the aggregation variable to ensure that it is possible to convert all fragments to their  canonincal forms. The canonical form of a fragment is such that:

* The fragment's data, in its entirety, must provide the values for a unique, contiguous part of the aggregated data.

* The fragment's data must have the same number of dimensions as the aggregated data, and each of those dimensions must uniquely correspond to an aggregated dimension, and be in the same order.

* Each dimension of the fragment's data must have the same sense of directionality (i.e. the sense in which it is increasing in physical space) as its corresponding aggregated dimension.

* The fragment's data must have the same data type as the aggregated data.

* The fragment's data must have the same units as the aggregated variable. 

* The fragment's data must have the same indication of missing values as the aggregated variable.

The conversion of fragments to their canonical form is the responsibility of the application program which is creating the aggregated data, and could include any of the following operations:

* Inserting any omitted size 1 dimensions into the fragment's data.

* Converting the fragment's data to have the aggregated variable's units.

* Changing occurrences of missing values in the fragment's data to values indicated by the aggregation variable as missing.

* Casting the data type of the fragment's data to the aggregated data type.

An application may choose to implement only a subset of these operations, but it will not be able create the aggregated data if can't convert all of the fragments to their canonical form.