...
An uncertainty ancillary construct (<<figure-field>>) provides metadata for describing the probability distribution of the measurand (i.e. the true unknowable value of a measurement stored in the field construct's array), and which are distributed over the same sampling domain as the field itself.
For instance, an uncertainty ancillary variable could provide a statistical moment of the probability distribution (such as skewness), or the joint probability distribution of the measurands (such as error-correlation coefficients).
An uncertainty ancillary construct consists of the following: 

* An optional array that depends on a subset of zero or more domain axis constructs.
...
* The array may also span extra trailing dimensions, one for each of domain axis constructs that the array spans with the same size, and in the same order, used to describe the joint probability distribution of the measurands at each location.

CF distribution parameter variables, error-correlation variables, and  error-correlation parameter variables correspond to uncertainty ancillary constructs. A error-correlation parameter given as an integer within an **`error_correlation`** string also corresponds to uncertainty ancillary construct.