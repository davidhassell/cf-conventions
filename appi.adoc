...
* Uncertainty constructs containing uncertainty metadata defined over the same domain.
* Uncertainty ancillary constructs containing ancillary metadata describing the probability distribution of the uncertainty metadata, and defined over the same domain.
...
image::images/mesh_figure.svg[,25%,pdfwidth=25vw,align="center"]
...
The field ancillary construct consists of an array of the ancillary data which is either scalar or which depends on one, more or all of the domain axis constructs.
...
It may have properties to describe the data (in the same sense as for the field construct).
...
The field ancillary construct may have cell methods constructs (<<data-model-cell-method>>), which are supplementary information for a single one of the cell method constructs of the field, but do not contribute to the definition of the domain.
...
The uncertainty construct (<<figure-field>>) provides metadata that describe a component of the uncertainty in the field construct's data, and which are distributed over the same sampling domain as the field itself.
It consists of the following:

* An array that defines the coverage interval within which the field construct's true data values occur with a known probability.
The array depends on zero or more of the domain axis constructs, with the addition of a size-two dimension for defining the interval limits.
It is assumed that the data do not depend on axes of the domain which are not spanned by the array, along which the values are implicitly propagated.

* Properties to describe the data (in the same sense as for the field construct).
The properties must include a "coverage probability" property to indicate the probability that a true value of the field construct's data lies in the interval given by the array.

* The uncertainty ancillary constructs that describe the probability distribution from which the coverage interval is derived.

CF uncertainty variables correspond to uncertainty constructs.

[[data-model-uncertainty-ancillary]]
==== Uncertainty ancillary construct

An uncertainty ancillary construct (<<figure-field>>) provides metadata for describing a parameter of the probability distributions of the measurands (i.e. the true unknowable values of measurements stored in the field construct's array).
The parameter is distributed over the same sampling domain as the field itself.
For instance, an uncertainty ancillary construct could provide a statistical moment of the probability distributions (such as skewness), or a descriptor of the joint probability distribution of the measurands (such as error-correlation coefficients).

An uncertainty ancillary construct consists of the following:

* Properties that describe how the parameter relates to the probability distributions.

* An optional array of parameter values that depends on a subset of zero or more domain axis constructs.
The array is omitted when the exact parameter values are not known.
The array provides values that describe the probability distribution at the locations indexed by the domain axis constructs.
When the joint probability distribution of the measurands is being described, the array may also span extra trailing dimensions, one for each domain construct spanned by the array, and in the same order.
Each extra dimension has the same size as its corresponding domain axis construct.

* Optional properties to describe the data array (in the same sense as for the field construct).

CF distribution parameter variables and error-correlation variables correspond to uncertainty ancillary constructs.
An error-correlation parameter variable corresponds to a property that describes how the parameter relates to the probability distributions; as does a comment, or an integer error-correlation parameter, within an **`error_correlation`** attribute.
